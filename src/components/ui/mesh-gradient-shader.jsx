...
// Customized Shader Colors to match Unity Financial Solution Brand
// Primary Navy: #0D1B3D -> 0.051, 0.106, 0.239
// Deep Green: #0E4D3A -> 0.055, 0.302, 0.227
// Gold Accent: #C89D3C -> 0.784, 0.616, 0.235
// Light Gray: #F2F4F7 -> 0.949, 0.957, 0.969
const FRAG = [
  "precision highp float;",
  "uniform vec2 u_res; uniform float u_time; uniform float u_speed; uniform float u_intensity; uniform float u_grain;",
  "const vec3 C_PRIMARY = vec3(0.051, 0.106, 0.239);", // Navy
  "const vec3 C_ACCENT  = vec3(0.055, 0.302, 0.227);", // Green
  "const vec3 C_PINK    = vec3(0.784, 0.616, 0.235);", // Gold
  "const vec3 C_MAGENTA = vec3(0.051, 0.106, 0.239);", // Navy (Blend)
  "const vec3 C_DEEP    = vec3(0.020, 0.050, 0.120);", // Darker Navy
  ...
  "  col = mix(col, vec3(0.055, 0.302, 0.227), 0.10*u_intensity*sin(t+uv.x*3.0));",
...
export function MeshGradient({
  ...
  useEffect(() => {
    ...
    if (!gl) {
      canvas.style.background = "#0D1B3D";
      return;
    }
